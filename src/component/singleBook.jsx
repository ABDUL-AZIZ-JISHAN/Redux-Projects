...
import { bookFormUpdateAction } from "../redux/bookUpdate/action";
const SingleBook = ({ book }) => {
  const dispatch = useDispatch();
  const { imgUrl, isFeatured, name, author, price, rating , id} = book;
    ...
    <div className="book-card">
      ...
      <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <div className="text-gray-500 space-x-2">
            <button onClick={()=> {
              ...
              dispatch(bookFormUpdateAction(book));
              }} className="lws-edit">
              <svg
                fill="none"
                viewBox="0 0 24 24"
            ...
            <button onClick={()=> dispatch(deleteBook(id))} className="lws-delete">
              <svg
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
        ...
        <div className="space-y-2 mt-4 h-full">
          ...
          <p className="lws-author">{author || "Dr Elizabeth McNaught"}</p>
          ...
          <p className="lws-price">BDT {price || 14}</p>
        </div>
      </div>
    </div>
  );
